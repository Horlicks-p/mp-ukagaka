...
- **API キー暗号化を AES-256-GCM へアップグレード**：新規 API キーは認証付き AES-256-GCM（`mpu_enc2:` プレフィックス）で保存されます。可逆な難読化 fallback と「AUTH_KEY 欠如時に site URL から鍵を導出する」安全でない経路を削除し、AUTH_KEY または OpenSSL/GCM が利用できない場合は fail-closed（保存を拒否）します。旧来の `mpu_enc:` / `mpu_obf:` / 平文も復号可能で、次回の設定保存時に GCM へ再暗号化されます。
- **LLM プロンプトのデバッグログ保護**：完全な system/user プロンプトと会話内容は `WP_DEBUG` だけでは出力されなくなりました。`MPU_DEBUG_LLM` 定数または `mpu_debug_llm_prompts` filter による明示的な opt-in が必要となり、プロンプト資産と会話 PII がデバッグログへ漏れるのを防ぎます。

---

## [2.25.5] - 2026-06-10

### 🛡️ セキュリティと安定性の強化（S級修正）

- **Touch API セッショントークン保護**：`/touch/decoration` および `/touch/zone` エンドポイントに、Chat システムと同様のセッショントークン検証を追加し、不正な API 呼び出しを防ぎます。
- **API キャッシュキーの完全性**：`mpu_generate_cache_key()` を修正し、`model`、`language`、`max_tokens` などの設定をハッシュ計算に含めるようにしました。これにより、設定変更後に古いキャッシュがヒットする問題を回避します。
- **オプションのディープマージ**：`mpu_get_option()` における `bot_blocker` や `extend` などのネストされた設定の浅いマージ（shallow merge）問題を修正しました。ネストされた配列の新しいデフォルト値が正しく保持されるようになります。
...
- Open-Meteo の明日予報に対し、`precipitation_sum` を使った雨勢ラベル補正を追加しました。連続雨系の WMO code が実際の24時間累積雨量より弱く見える場合、予報ラベルを雨量に合わせて引き上げ、「霧雨」と過小表現しないようにしました。
- 現在天気は日別累積雨量で上書きせず、リアルタイムの WMO code を維持します。コンテキストには当日の累積雨量を添えるため、キャラクターは実際の雨量を把握しつつ、今この瞬間を大雨と誤認しません。
- 極端な雨量段階のラベルをより自然な日本語へ調整し、Frieren の bot detection テンプレート1件も矛盾した比喩が出ない表現へ書き換えました。
- Gift/Feeding 実装計画に、normalizer context の所在、初回 interaction の history 取り扱い、observation dedupe wording について、実コード確認済みの補足を反映しました。
