            ...
            <div>
                ...
                <p>
                    ...
                    <?php if ($key == str_replace('default', '', $key)) { ?>
                        / <a href="<?php echo site_url(); ?>/wp-admin/options-general.php?page=<?php echo plugin_basename('mp-ukagaka/options.php'); ?>&cur_page=1&del=<?php echo $key; ?>">[<?php _e('刪除', 'mp-ukagaka'); ?>]</a>
                    <?php } ?>
                </p>

                <!-- 選項：是否可顯示 -->
                <p><label><input type="checkbox" name="ukagakas[<?php echo $key; ?>][show]" value="true"<?php if ($value['show']) { echo ' checked="checked"'; } ?> /><?php _e('可顯示', 'mp-ukagaka'); ?></label></p>
