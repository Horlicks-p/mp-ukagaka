...
- Open-Meteo 明日預報現在會使用 `precipitation_sum` 校正雨勢標籤。當連續雨系 WMO code 低估實際 24 小時累積雨量時，預報標籤會依雨量提高，避免把明顯不小的雨說成「霧雨」。
...
- 新增 Response Normalizer 契約。AI 回覆的顯示文字、history 文字、checksum 文字與 TTS 文字現在共用同一份清理後輸出，開頭 `<think>` / `<thinking>` 與 emotion tag 則解析成結構化欄位。
- 新增 inline `[tag]` 表情提示規則。Frieren 於 `manifest.json` 宣告支援的 emoji tag 清單，取代舊的句尾 `[表情:xxx]` 提示風格，同時保留向下相容解析。
- 非串流 REST 與 SSE 串流輸出已接上 normalized emotion 管線。串流中明確出現的 emotion tag 會優先保留，不再於 done 階段被關鍵字猜測覆蓋。
- 新增 stream parser 測試，涵蓋 split tag、split think、Markdown link 偽陽性、context-disabled 路徑，以及 malformed / 未閉合 think block。

### Think bubble 與 system placeholder

- 新增共用 `#ukagaka_think` 氣泡，供 system placeholder 與未來 LLM think 輸出使用。`えっと`、初始 `何を話せばいいかな` 等等待提示現在會顯示於主對話框外。
- 修正 Frieren touch / decoration / initial loading 流程，placeholder bubble 不進 chat history、不留下 stale placeholder attribute，也不會在第一句真實台詞出現前閃出空白主對話框。
- 調整 think bubble 視覺與定位：右側錨定、文字往左延伸、箭頭方向與位置微調、虛線邊框，並讓透明度與主對話框視覺一致。
...
- 前端所有 console log 字串已從硬編碼繁體中文遷移為日文 source 字串，並透過 WordPress locale 顯示對應翻譯。`js/` 與 `ghost/` 中已無任何硬編碼 CJK console log。
- 遷移涵蓋全部 console log：production-visible 部分（`js/ukagaka-anime.js`、`ghost/Frieren/frieren.js`，以及 core bundle 收尾的 `js/ukagaka-features.js` / `js/ukagaka-base.js`），以及散布於 core bundle、Frieren、dialog、greeting、emoji 模組的 195 條 debug-gated log。
- log 現在統一經由 `mpuLogger` 輔助方法（`logL` / `logF` / `warnL` / `warnF`，以及一律輸出的 `errorL` / `errorF` / `warnAlways` / `warnAlwaysF`），並以穩定的語意 key 索引。字串分兩個 bucket 注入：`mpuL10n.logs`（一律注入）與 `mpuL10n.logsDebug`（僅在前端 debug 模式注入）。
- 輸出時機與 gating 維持不變：error/warn 輸出照舊，debug-only log 仍僅在管理員開啟 `WP_DEBUG` 時顯示。

### 🗂️ 翻譯目錄

- 重新產生 `mp-ukagaka.pot` 並合併 `ja`、`zh_TW`、`en_US` 目錄。日文與繁體中文已完成全部 206 條 unique log msgid 的翻譯（212 條註冊，其中 6 條共用相同日文 source）。英文（`en_US`）已合併並編譯，但尚未翻譯，因此英文 locale 網站這些 log 會 fallback 顯示日文 source 字串。
