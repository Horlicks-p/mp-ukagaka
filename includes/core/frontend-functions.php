	...
	if ( function_exists( 'mpu_is_deep_sleep_time' ) && mpu_is_deep_sleep_time( $personality_id ) ) {
		...
		if ( $personality_id !== null && function_exists( 'mpu_load_personality_sleep_mode' ) ) {
			...
			if ( ! empty( $sleep_config['enabled'] ) ) {
				...
				$visitor_info    = function_exists( 'mpu_get_visitor_info_for_llm' ) ? mpu_get_visitor_info_for_llm() : array();
...
function mpu_enqueue_frontend_assets() {
	...
	if ( $log_i18n ) {
		...
		$log_i18n->always( 'frierenDecorationConfigLoadFailed', __( '装飾設定を読み込めませんでした：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. Used when decoration config loading fails without a server-provided error message. */
		$log_i18n->always( 'frierenDecorationConfigLoadFailedUnknown', __( '装飾設定を読み込めませんでした：不明なエラー', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. %s is the jQuery AJAX error value for loading decoration config. */
		$log_i18n->always( 'frierenDecorationConfigAjaxFailed', __( 'AJAX による装飾設定の読み込みに失敗しました：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. Required front-end runtime dependencies for decoration config loading are unavailable. */
		$log_i18n->always( 'frierenDecorationConfigRuntimeUnavailable', __( 'jQuery または mpuurl が利用できないため、装飾設定を読み込めません', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. The decoration_config structure is not in the expected format. */
		$log_i18n->always( 'frierenDecorationConfigInvalid', __( '装飾設定が無効です', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. shellInfo contains the character image URL and display settings. */
		$log_i18n->always( 'frierenShellInfoInvalid', __( 'フリーレンモード：shellInfo が無効です', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		...
		$log_i18n->always( 'frierenPixelCanvasCreateFailed', __( 'ピクセル判定用 Canvas を作成できません：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. %1$s is the decoration type, %2$s is the exception message. */
		$log_i18n->always( 'frierenPixelDataUnavailable', __( 'ピクセルデータを取得できません（クロスオリジンの可能性があります）：タイプ=%1$s、メッセージ=%2$s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. The request error object is logged as an additional console argument. */
		$log_i18n->always( 'frierenTouchZoneDialogRequestFailed', __( 'タッチ領域の会話リクエストに失敗しました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: console log. The decoration dialog request error object is logged as an additional console argument. */
		$log_i18n->always( 'frierenDecorationDialogRequestFailed', __( '装飾品会話リクエストに失敗しました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		...
		$log_i18n->debug( 'contextTriggerConditionsCheck', __( 'mpu_check_page_trigger: チェック条件 = %1$s、path = %2$s', 'mp-ukagaka' ) );
		...
		$log_i18n->debug( 'frierenEmojiConfigLoadFailed', __( 'mpuEmojiManager: 表情設定を読み込めませんでした：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Emoji base path is not configured. */
		$log_i18n->debug( 'frierenEmojiBasePathMissing', __( 'mpuEmojiManager: 表情のベースパスが設定されていません', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. The ukagaka image container is missing. */
		$log_i18n->debug( 'frierenEmojiContainerMissing', __( 'mpuEmojiManager: #ukagaka_img コンテナが見つかりません', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console warning. %s is the failed emoji image URL. */
		$log_i18n->debug( 'frierenEmojiImageLoadFailed', __( 'mpuEmojiManager: 表情画像の読み込みに失敗しました：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		...
		$log_i18n->debug( 'frierenDecorationConfigLoaded', __( 'JSON 設定から %s 個の装飾品を読み込みました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %s is the decoration type whose transparent area was clicked. */
		$log_i18n->debug( 'frierenDecorationTransparentClickIgnored', __( '透明領域がクリックされたため無視します：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %s is the decoration type hit by pixel detection. */
		$log_i18n->debug( 'frierenDecorationPixelHitClicked', __( '装飾品がクリックされました（ピクセルヒット）：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %1$s is the decoration type, %2$s is the canvas size. */
		$log_i18n->debug( 'frierenPixelDetectionCanvasCreated', __( 'ピクセル検出 Canvas を作成しました：%1$s、%2$s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Values are decoration type, pixel coordinates, alpha, and hit threshold. */
		$log_i18n->debug( 'frierenPixelDetectionSample', __( 'ピクセル検出：%1$s、x=%2$s、y=%3$s、alpha=%4$s、threshold=%5$s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		...
		$log_i18n->debug( 'frierenPostWakeBookFlipPlaying', __( '📖 目覚め後にページめくりアニメーションを再生します', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Book-flip animation is skipped after wake. */
		$log_i18n->debug( 'frierenPostWakeBookFlipSkipped', __( '📖 目覚め後のページめくりアニメーションをスキップします', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Book-flip animation is skipped during sleep mode. */
		$log_i18n->debug( 'frierenSleepModeBookFlipSkipped', __( '🌙 睡眠モード：ページめくりアニメーションをスキップします', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Book-flip animation is skipped for manual wake dialogue. */
		$log_i18n->debug( 'frierenManualWakeDialogueBookFlipSkipped', __( '📖 手動の目覚め会話：ページめくりアニメーションをスキップします', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %s is the decoration type passed to the click handler. */
		$log_i18n->debug( 'frierenDecorationClickHandled', __( 'handleDecorationClick が呼び出されました。装飾品タイプ：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Decoration click is ignored while a decoration dialog is active. */
		$log_i18n->debug( 'frierenDecorationClickIgnoredDialogActive', __( '装飾品会話中のため、今回のクリックを無視します', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Decoration dialog is skipped because AI is disabled. */
		$log_i18n->debug( 'frierenDecorationDialogSkippedAiDisabled', __( 'AI 機能が有効ではないため、装飾品会話をスキップします', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Additional argument is the cancelled decoration click request ID. */
		$log_i18n->debug( 'frierenDecorationClickRequestCancelled', __( '装飾品クリック：リクエストをキャンセルしました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Decoration dialog completed and state was restored. */
		$log_i18n->debug( 'frierenDecorationDialogCompletedStateRestored', __( '装飾品会話が完了し、状態を復元しました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %1$s is the zone name, %2$s is the relative Y coordinate. */
		$log_i18n->debug( 'frierenTouchZoneDetected', __( 'タッチ領域検出：%1$s、relativeY=%2$s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %s is the handled touch zone name. */
		$log_i18n->debug( 'frierenTouchZoneHandled', __( 'handleTouchZone が呼び出されました。領域：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %s is the touch zone currently in cooldown. */
		$log_i18n->debug( 'frierenTouchZoneClickIgnoredCooldown', __( '領域がクールダウン中のため、クリックを無視します：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %s is the touch zone entering cooldown after reaching its click limit. */
		$log_i18n->debug( 'frierenTouchZoneClickLimitReached', __( '領域のクリック上限に達したため、クールダウンに入ります：%s', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Additional argument is the cancelled touch zone request ID. */
		$log_i18n->debug( 'frierenTouchZoneRequestCancelled', __( 'タッチ領域クリック：リクエストをキャンセルしました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. Character touch event handlers have been bound. */
		$log_i18n->debug( 'frierenTouchEventsBound', __( 'キャラクターのタッチイベントを設定しました', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
		/* translators: debug console log. %1$s is the touch zone name, %2$s is the cooldown duration in seconds. */
		$log_i18n->debug( 'frierenTouchZoneCooldownStarted', __( '領域がクールダウンに入りました：%1$s、クールダウン時間：%2$s 秒', 'mp-ukagaka' ), array( 'scope' => 'frieren' ) );
	...
	if ( mpu_is_frontend_debug_mode() ) {
		$l10n['logsDebug'] = $log_i18n ? $log_i18n->logs_debug() : array();
	}
