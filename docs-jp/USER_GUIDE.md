...
チャットモード中、入力ボックスに直接スラッシュコマンドを入力することができます。
...
> **注意**：管理者専用コマンドは WordPress のログイン状態によって判定されます。非ログインユーザーがこれらのコマンドを入力すると、キャラクターがキャラクターらしく拒否します。

#### `/help`

利用可能なコマンドの一覧を表示します。ログイン状態に関係なく全員が使用できます。

#### `/reset` と `/clear`

現在のページの会話履歴を即座に消去します。会話が脱線したときや、ページを更新せずにやり直したいときに便利です。

> **管理者のみ**：WordPress 管理者としてログインしている必要があります。

#### `/debug_mcp`

Abilities/MCP ツールシステムの状態を確認できる診断パネルを表示します：

- 統合モジュールの読み込み状態
- Manager クラスの状態
- 利用可能なツール数
- 各アビリティの状態（例：`get-popular-posts`、`get-bot-blocker-stats`、`ban-ip`、`clear-bot-blocker-data`）
