...
function mpu_build_item_reaction_prompt( array $item, $visitor_message, $ukagaka_name, array $reaction_pool = array(), $has_history = false ) {
	...
	// 食べる／中を確かめることは許可するが、既成事実にも義務にもしない.
	$prompt .= 'food' === $kind
		? "\n差し出された食べ物を受け取ること。その場で口をつけてもよい。"
		: "\n差し出された贈り物を受け取ること。中を確かめられる品なら、その場で確かめてもよい。";

	// 制止の例外は、相手がそれを言える場が実在するときだけ書く。附言も履歴も無い回で
	// 「変質や安全性への具体的な懸念を述べている場合」と書くと、参照先の無い警告を
	// 名指ししたことになり、モデルはそれを埋めにかかる（:310 と同じ理由）。実際に
	// 「食べる前に確認した方がいいかな。変な味がしないか」と、誰も言っていない懸念を
	// 自分で作り出した。細部の補完は下の規則で許可しているので、なおさら効く.
