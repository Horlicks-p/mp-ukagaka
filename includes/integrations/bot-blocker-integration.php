...
function mpu_bb_get_ip() {
    $remote_addr = isset($_SERVER['REMOTE_ADDR']) ? sanitize_text_field($_SERVER['REMOTE_ADDR']) : '0.0.0.0';

    // 安全策略：只有在 REMOTE_ADDR 是私有/保留 IP（即連線來自本機反向代理）時，
    // 才信任 proxy header。若 REMOTE_ADDR 是公開 IP，直接使用，不信任任何 proxy header。
    // 理由：無法在不維護 CDN IP 段清單的前提下，區分「可信 CDN 代理」與「繞過 CDN 直打
    // origin 並偽造 header 的攻擊者」。使用 REMOTE_ADDR 雖然在 CDN 情境下會得到
    // CDN 出口 IP，但可防止偽造公開 IP 觸發 auto-ban 誤封合法目標。
