...
**MP Ukagaka** is a WordPress plugin (v2.13.6) that creates interactive "ukagaka" (伺か) desktop-companion–style characters on WordPress sites. Characters display dialogue, respond to page content using AI APIs, and support multi-turn chat with visitors.
...
│   ├── ukagaka-base.js         # Base config & utilities (~953 lines)
│   ├── ukagaka-core.js         # Core character logic (~1,196 lines)
│   ├── ukagaka-chat.js         # Interactive chat mode (~861 lines)
│   ├── ukagaka-context.js      # Page awareness/context (~582 lines)
│   ├── ukagaka-features.js     # Settings & feature flags (~492 lines)
│   ├── ukagaka-anime.js        # Canvas animation (~440 lines)
...
Module load order in `mp-ukagaka.php` is **critical** and intentional. Dependencies are strict:

1. `debug-functions.php` — must be first (logging used by all others)
2. `core-functions.php` → `utility-functions.php`
3. `personality-loader.php` → `personality-prompts.php` → `personality-decorations.php` → `personality-emoji.php`
4. `stats-collector.php` → `stats-analyzer.php` (before `ai-functions.php`)
5. `api-cache.php` → `provider-helpers.php` → `chat-integrity.php` → `request-state.php` → `tool-loop-guard.php` → `streaming-helpers.php` → `provider-stream-http.php`
6. `providers/bootstrap.php` (before `ai-functions.php`)
7. `ai-functions.php` → `prompt-categories.php` → `llm-slimstat.php` → `llm-context-builder.php`
8. `weather-functions.php` → `diary-functions.php` → `llm-functions.php`
9. `emoji-mapper.php` → `ukagaka-functions.php` → `rest/bootstrap.php` → `ajax/chat-api-handlers.php`
10. Integration modules last

Frontend-only: `core/frontend-functions.php` (skipped when `is_admin()`)
Admin-only: `includes/admin-functions.php` (only loaded when `is_admin()`)
...
- **ukagaka-chat.js**: Chat mode UI, SSE stream consumption, history management
