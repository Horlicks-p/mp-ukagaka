...
> 💡 在 PowerShell 在 PowerShell 使用 `ollama list` 命令查看已下載的模型
...
1. **找到提示詞定義位置**
   ...
   - 函數：`mpu_generate_llm_dialogue()`
   - 約在第 255-300 行

2. **提示詞分類結構**

   提示詞分為多個類別，系統會隨機選擇一個類別，然後從該類別中隨機選擇一個提示詞：
...
3. **WordPress 資訊整合功能**
   ...
   **統計資訊（遊戲化用語）：**
   - **魔族遭遇回數**（文章篇數）：`{$post_count}`
   - **最大傷害**（留言數量）：`{$comment_count}`
   - **習得スキル總數**（分類數量）：`{$category_count}`
   - **アイテム使用回數**（TAG數量）：`{$tag_count}`
   - **冒險日數**（運營日數）：`{$days_operating}`
...
4. **個人化統計資訊提示詞範例**
   ...
   'statistics' => [
       ...
       "習得したスキル（カテゴリー）は{$category_count}個ある、これについて軽く言う",
   ];
   ...
   **提示詞設計要點：**
   ...
   - ✅ **明確說明統計項目**：在提示詞中明確提到是「魔族に遭遇した回数」「与えたダメージ」等完整描述，避免只說數字讓使用者摸不著頭緒
   - ✅ **結合角色設定**：將統計資訊轉化為符合角色世界觀的用語（例如「魔族」「アイテム」「スキル」）
   - ✅ **自然表達**：使用「について軽く言う」「について一言」等自然指令，讓 AI 生成更自然的對話
   - ❌ **避免只說數字**：不要只說「{$post_count}回」，要說明這是什麼的回數
...
- ✅ **避免矛盾**：確保同一類別的提示詞風格一致
- 💡 **長度建議**：
  - **雲端 AI 服務**（Gemini、OpenAI、Claude）：建議 200-300 字以內，以控制 API 成本
