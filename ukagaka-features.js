...
jQuery(document).ready(function(){
    ...
    mpuFetch(settingsUrl, {
        ...
        .then(res => { // success:
            ...
            // ★★★ 保存 LLM 取代對話設定 ★★★
            if (typeof res.ollama_replace_dialogue !== 'undefined') {
                mpuOllamaReplaceDialogue = res.ollama_replace_dialogue === true;
                mpuLogger.log('LLM 取代對話設定: ' + (mpuOllamaReplaceDialogue ? '啟用' : '停用'));
            }
            
            // ★★★ 修改：只有在未啟用 LLM 取代對話時才載入外部對話 ★★★
            if (!mpuOllamaReplaceDialogue) {
                initExternalDialog();
            } else {
                mpuLogger.log('LLM 取代對話已啟用，跳過外部對話載入');
                // 如果啟用了 LLM 取代對話，顯示初始訊息
                mpu_typewriter("（思考中...）", "#ukagaka_msg");
                if (mpuAutoTalk) startAutoTalk();
            }
    ...
    // 4. 外部檔案對話初始化（移到獲取設定後，由設定決定是否載入）
    // ★★★ 修改：將外部對話初始化移到獲取設定後，避免在 LLM 模式下載入外部對話 ★★★
