...
- **The three buttons under the character are inline SVG instead of five PNGs.** The dock was `menu.png` as a background sprite for the idle row, with `top.png` / `hide.png` / `change.png` swapped in as hover backgrounds. Each button now carries one `<svg>` from `mpu_get_dock_icon_svg()`, and the idle and hover states are custom properties — `--mpu-internal-dock-ring`, `-disc`, `-glyph`, `-halo`, `-shadow` and `-shift` — so a single drawing covers both. The buttons are crisp at any pixel density, and the dock stops issuing five image requests.
- **The artwork was traced from the bitmaps rather than redrawn by eye.** Each glyph was recovered from the source pixels — the 45-degree triangle, the door frame with an arrow leaving through its right edge, the screwdriver crossed with a combination spanner. Every radius and stroke width comes from the half-width of the corresponding band in the source's radial alpha profile: the heavy ring at `r=12.0..16.7`, the outer at `r=18.75..21.05`. Rendered at 1:1 and diffed against the PNGs, the six icons sit 353 differing pixels from the originals at a mean channel error of 7.74, with idle (2.3–3.6%) level with hover (1.4–3.5%).
- **The colours are the source bitmap's own RGBA, so the drawing matches on any page background.** The rings are `rgb(250 250 250 / 65%)` — the value measured off `menu.png` — which composites to `#FCFCFC` on white and `#AEAEAE` on `#222`, matching the original in both. They stay translucent on purpose: an opaque grey reads on a white page but inverts the original's light-ring/dark-disc order once the page behind it is dark, since a ring and disc sharing one base colour and differing only in alpha track the background at different rates.
- **The faint dark halo the bitmap baked between the two rings is drawn explicitly.** The near-white rings are legible only because of it, and vector strokes have none, so it is a `--mpu-internal-dock-halo` stroke that the hover rule clears — the three hover bitmaps measure a fully transparent gap.
- **`menu.png`'s shadow and its 1px offset are reproduced, and only on idle.** The bitmap casts a one-sided shadow: the darkening sits at 178°, essentially straight left with no vertical drop, 218 on the left flank against 254–255 on the right. Its circles are also centred at `(23.5, 23.0)` while `top.png` / `hide.png` / `change.png` sit at `(22.5, 22.5)` — the two source sets disagree by a pixel, and one drawing cannot hold both centres. The geometry keeps 22.5, correct for hover, and idle makes up the difference with a `transform`; both the shadow and the shift are custom properties the hover rule already clears. Using the PNGs this pixel showed up as a twitch on hover, which sharing one drawing removes.
...
- **Every cue must be expressible in what she says.** The reply is her own speech, 30–150 characters, with third-person description forbidden (`personality-items.php:313`), so a cue resting on a channel the reply cannot carry has no legal rendering — the model either narrates ("she smiled, then went blank again") in breach of that rule, or fills the space with ellipses. Three cues did this: silence held after receiving, an expression softening and being put back, and hands stopping. All three are rewritten onto tone, word choice, and how much she says. The constraint is stated in `_comment_give` alongside the cross-kind one.
...
- **Gifts now happen inside the conversation.** `/touch/give` already received, verified, and stored the chat history, but the reaction itself was generated through `mpu_call_ai_api()`, which takes a single prompt string and no messages — so the history never reached the model. Handing Frieren something mid-conversation was, from her side, an isolated event with no memory of what had just been said; telling her "this is a thank-you gift" in the previous turn could not possibly affect the reply. The reaction now goes through `mpu_call_ai_api_with_messages()` with the same 20-message window `/chat/user` uses. The window width matches deliberately: a narrower one would give the character a different memory during a gift than during the sentence before it.
- **The prompt no longer mixes fact, motive, and stage direction.** All three were concatenated as equally authoritative imperatives, so a randomly drawn line like "wonder a little about why they picked this" outranked the visitor plainly stating they had no idea what the item was. The rules block now assigns each source a **jurisdiction** rather than a rank: `items.json` decides what was handed over and what the character observed, the visitor's words and the conversation decide why, from where, and how much they knew, and `prompts.json` decides only how the character behaves. A single ranking would have been wrong in the other direction — putting the visitor's words above the catalog means handing over the grimoire while insisting "this is a sword" makes it a sword. Only a genuine ordering problem is stated as one: when this turn contradicts an earlier turn, this turn wins. Two prohibitions are added — do not treat something the visitor said they did not know as known, and do not invent a reason they never gave — and the angle is labelled a candidate to be discarded when it contradicts the conversation. The rules are stated in text rather than implied by ordering, because imperatives are obeyed wherever they sit.
- **Jurisdiction is declared only for sources that exist.** With no prior turns, or no attached message, the corresponding clause is omitted entirely. Naming a block that is not there invites the model to treat it as information it was supposed to have received and fill it in.
- **The grimoire's hidden variant is attributed to Frieren, not to the visitor.** The catalog asserted `それは{variant}だった` in narrator voice and then ordered `中身に具体的に触れて反応すること`, which turned a server-side random draw into shared knowledge. It now reads as what Frieren *would* find on inspecting it, states outright that the visitor may not have known, and leaves whether she opens it at all to the turn's own conversation — asserting the inspection as already done would have been the same defect as the forced tasting, just rarer ("don't open it yet, it's sealed"). The variants themselves are unchanged.
- **Food is no longer eaten by decree.** `受け取って食べ、味の感想を述べること` forced both the act and its outcome, so "careful, this might be past its date" still produced a verdict on the taste. Tasting is now conditional on the visitor not having warned against it.
- **Four reaction lines rewritten.** In `give_gift` and `give_favorite`, two lines presupposed a deliberate choice by the visitor and two `give_food` lines presupposed that eating had already happened. Since the draw occurs before the visitor's message is read, no line may presuppose anything about the visitor at all; `_comment_give` now states that rule for future ghosts.
- **Repeat gifts stop replaying verbatim.** The single-prompt path runs through the API response cache, and メルクーアプリン has no variants — with caching enabled, its prompt had only 8 possible forms, so feeding it twice within the TTL returned a byte-identical reply. The multi-turn path is not cached, and the conversation window differs every turn regardless.

### Internal

- **`MPU_Chat_History_Service::filter_orphan_assistants()` and `::to_llm_messages()`**: the orphan-assistant rule (drop an `assistant` with no preceding `user` anchor, *before* slicing the window) lived inline in the chat controller. `/touch/give` needs the same rule, and a second hand-written copy is exactly how the message-type allowlist drifted and broke checksums previously, so both callers now share one implementation.
- **`mpu_build_item_reaction_prompt()`**: prompt assembly moved out of the REST controller into `personality-items.php`, where it is unit-testable. Eight new tests cover the conflict cases that motivated the change — visitor does not know the contents, visitor warns against eating, visitor contradicts an earlier turn, visitor misnames the item, no message at all, no history at all, and the data-level guarantees about the reaction categories and the grimoire entry.
- **`tools/node/test-gift-transport-smoke.js`** (wired into `verify`): the unit tests cover the helpers in isolation, so every helper can be correct while the controller quietly stops calling them — which is precisely the shape of the bug this release fixes, and nothing failed at the time. The smoke test asserts the wiring: history becomes provider messages, the gift prompt is appended last, the messages reach `run_reaction`, the multi-turn branch precedes the cached single-prompt fallback, the tool-suppression filter is removed in a `finally`, and both gift and chat read the same 20-message window. These are source-level assertions, not runtime ones; there is no WordPress harness here.
- **New `mpu_mcp_tools_for_llm` filter**: the role whitelist decides *who may* use an ability; this filter decides *whether this call offers any*. A one-shot reaction has no use for tools, but an administrator browsing their own site would have been handed every tool definition — wasted tokens, and a chance for the model to answer with `tool_use` on a path that has no tool-loop handling. `/touch/give` returns an empty array for the duration of its call.
